 ...
 * (c) 2014-2016 Pavel Reznikov, Dylan Weiss, Prashik
...
(function(factory) {
    ...
    var Utils = {
        ...
        //plugin customization prashik start
        sort: function (nodes, dir, width, gllangType) {
            //plugin customization prashik end
            width = width || _.chain(nodes).map(function(node) { return node.x + node.width; }).max().value();
            dir = dir != -1 ? 1 : -1;
            //plugin customization prashik start
            if (gllangType == "ar") {
                return _.sortBy(nodes, function (n) { return (dir * -1) * (n.x - n.y * width); });
            }
                //plugin customization prashik end
            else
            {
    ...
    */
    ...
    //plugin customization prashik start
    GridStackEngine.prototype._sortNodes = function (dir, gllangType) {
        this.nodes = Utils.sort(this.nodes, dir, this.width, gllangType);
        //plugin customization prashik end
    };
    ...
    var GridStack = function(el, opts) {
        ...
        this.onResizeHandler = function() {
            ...
            if (self._isOneColumnMode() && !self.opts.disableOneColumnMode) {
                ...
                oneColumnMode = true;
                //plugin customization prashik start
                self.grid._sortNodes(gllangType);
                //plugin customization prashik end
    ...
    GridStack.prototype._updateStyles = function(maxHeight) {
        ...
        //plugin customization prashik start
        if (!onlyHeightOP && this._styles._max !== 0 && maxHeight <= this._styles._max) { // Keep this._styles._max increasing
            return;
        }
        //plugin customization prashik end
        this._initStyles();
        ...
        //plugin customization prashik comment start
        if (this._styles._max !== 0 && maxHeight <= this._styles._max) {
            return ;
        }
        //plugin customization prashik comment end
    ...
    GridStack.prototype._updateContainerHeight = function() {
        ...
        var maxBottom = 0;
        if(typeof fldsHideOnPage!="undefined" && fldsHideOnPage=="true"){
    this.container.children('.' + this.opts.itemClass + ':not(.d-none)').each(function() {
        var $item = $(this);
        var y = parseInt($item.attr('data-gs-y')) || 0;
        var h = parseInt($item.attr('data-gs-height')) || 1;
        maxBottom = Math.max(maxBottom, y + h);
    });
}

    // Fallback to grid height if no visible items
    var height = maxBottom || this.grid.getGridHeight();
    ...
    GridStack.prototype._prepareElementsByNode = function(el, node) {
        ...
        var onEndMoving = function(event, ui) {
            ...
            //plugin customization prashik check start
            if (event.type == 'resizestop') {
                self.container.trigger('gsresizestop', o);
            }
            //plugin customization prashik check stop
        };

 ...
 * Modified:    March 2021 by Prashik Nandeshwar - prashik.nandeshwar@gmail.com
 * 1. Added fix for increasing column width out of table(table width increases) with scrollX enabled.
 * Language:    Javascript
 * License:     MIT
 * Project:     DataTables
 *
        ...
        if(this.dom.resize) {
            ...
            // Smart scrolling is enabled.
                ...
                if(scrollXEnabled) {
                    ...
                    var useTheadLength = $scrollBody.closest('.dataTables_scroll').find('.dataTables_scrollHead table thead').first().length == 1;
                    // if((this.table_size === undefined || this.table_size < 0) && $scrollHeadTableWrapper.length > 0) {
                        if(useTheadLength){
                            this.table_size = $scrollBody.closest('.dataTables_scroll').find('.dataTables_scrollHead table thead').width();
                        }else{
                ...
                if(moveLength < 0) {
                    ...
                    $scrollBodyNTh.width(nThWidth);
                    if (scrollXEnabled) {
						//this exact code is available below this if block
                        ...
                        $scrollBody.closest('.dataTables_scroll').width(new_table_size);
                ...
                if (!scrollXEnabled) {
					//this exact code is available above this if block
                    var new_table_size = this.table_size;
                    new_table_size += moveLength;
                    // Resize the header too (if sScrollX is enabled).
                    $($scrollHeadInner).width(this.table_size + moveLength);
        ...
        if ( this.dom.drag !== null )
        {
            ...
            if(scrollXEnabled && $(this.s.dt.nScrollHead, this.s.dt.nTableWrapper) != undefined){
                if($(this.s.dt.nScrollHead, this.s.dt.nTableWrapper).length > 0){
                    var useTheadLength = $(this.s.dt.nScrollHead).closest('.dataTables_scroll').find('.dataTables_scrollHead table thead').first().length == 1;
                    if(useTheadLength){
                        this.table_size = $(this.s.dt.nScrollHead).closest('.dataTables_scroll').find('.dataTables_scrollHead table thead').first().width();
                    }else{
			...
			$(this.s.dt.nScrollHead).find('div.dataTables_scrollHeadInner').width(this.table_size);
            $(this.s.dt.nScrollHead).closest('.dataTables_scroll').width(this.table_size);
            $(this.s.dt.nScrollHead).closest('.dataTables_scroll').find('.dataTables_scrollHead table').first().width(this.table_size);
            $(this.s.dt.nScrollHead).closest('.dataTables_scroll').find('.dataTables_scrollBody table').first().width(this.table_size);

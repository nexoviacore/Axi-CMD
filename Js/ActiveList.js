...
class AxTasks {
    constructor() {
        ...
        this.taskBtnsHtml.view = `
            <a href="javascript:void(0)" title="View form" class="btn btn-white btn-color-gray-700 btn-active-primary d-inline-flex align-items-center  btn-sm  me-2 " onclick="axTasksObj.openViewForm('{{transid}}','{{recordid}}','{{processname}}','{{keyvalue}}','{{taskname}}')"><span class="material-icons material-icons-style material-icons-2" style="color: darkmagenta;">visibility</span> View form
...
`;
        ...
        this.taskBtnsHtml.check = `
             <a href="javascript:void(0)" data-confirmMessage='' title="Check" class="btn btn-white btn-color-gray-700 btn-active-primary d-inline-flex align-items-center  me-2  btn-sm" onclick="axTasksObj.doCheck('{{taskid}}')">
        ...
        <li>
            <a href="javascript:void(0)" title="View Form" onclick="axTasksObj.openViewForm('{{transid}}','{{recordid}}','{{processname}}','{{keyvalue}}','{{taskname}}')">
    ...
    isEditableTask(taskName, keyValue) {
        ...
        let data = { processName: _this.processName, taskName: taskName, keyValue: keyValue, indexNo: parent.axProcessObj.currentIndex };
        this.callAPI(url, data, false, result => {
            if (result.success) {
                let json = JSON.parse(result.response);
                let dataResult = _this.dataConvert(json, "ARM");
    ...
    isEditableForm(keyValue, recordId) {
        var isEditable = false;
        let _this = this;
        let url = "../aspx/AxPEG.aspx/AxGetEditableForm";
        let data = { processName: _this.processName, keyValue: keyValue, recordId: recordId };
        this.callAPI(url, data, false, result => {
            if (result.success) {
                let json = JSON.parse(result.response);
                let dataResult = _this.dataConvert(json, "ARM");
    ...
    openTstruct(transId, recordId, processName, keyValue, taskName) {
        ...
        if (this.isAxpertFlutter) {
            ...
            var isEditable = this.isEditableTask(taskName, this.keyValue || keyValue);
    ...
    openViewForm(transId, recordId, processName, keyValue, taskName) {
        let url = "";
        if (this.isAxpertFlutter) {
            this.checkAxpertConnection();
            url = `${axpertUrl}/aspx/AxMain.aspx?authKey=AXPERT-${armSessionId}&pname=t${transId}`;
            ...
            var isEditable = this.isEditableForm(this.keyValue || keyValue, recordId);
            let isPegEdit = `&ispegedit=${isEditable.toString()}`;

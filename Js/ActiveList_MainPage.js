var armToken = "";
var doTask;
var isActiveTskPopoverVisible = false;
...
class AxTasks {
        ...
        <table class="table align-middle  fs-6 gy-5 mb-0 dataTable no-footer No-Task-Containter">
            <tbody>
                <tr class="d-flex ">                                           
					...
					<!--<img src="../custompages/icons/notask.gif" >-->
    ...
    init() {
        this.getUrlParams();
        let _tasks = this.task;
        if (this.isNullOrEmpty(_tasks.taskId) || this.isUndefined(_tasks.taskId)) {
            this.fetchActiveTasks("Tasks");
        }
        else {
            document.querySelector("#ActiveTask_Left").classList.remove('d-flex');
            document.querySelector("#ActiveTask_Left").style.display = "none";
            this.fetchProcessTask("Tasks");
            if (!this.isUndefined(this.dataSources?.["Tasks"]?.[0]?.["displaytitle"])) {
                document.querySelector("#task-page-title").innerHTML = this.dataSources["Tasks"][0]["displaytitle"];
            }
        }
        this.showTasks();
        ShowDimmer(false);
    ...
    bulkInit() {
        ...
        this.getUrlParams();
        let _tasks = this.task;
        if (this.isNullOrEmpty(_tasks.taskName) || this.isUndefined(_tasks.taskName)) {
    ...
    bindEvents() {
        ...
        document.querySelectorAll(".sendto").forEach((item) => {
            KTMenu.getInstance(item).on("kt.menu.dropdown.show", function (item) {
                let itemData = item.dataset;
                let sendToDropDown = document.querySelector(`[data-popoverid="SEND${itemData.taskid}"] select`);
                if (sendToDropDown != null && sendToDropDown?.classList.contains("data-fetched") == false) {
                    _this.getSendToUsers(itemData.taskid, itemData.tasktype, itemData.keyvalue, itemData.taskname, sendToDropDown);
                }
            });
        });
        ...
        document.querySelectorAll(".taskActionBtn").forEach((item) => {
            KTMenu.getInstance(item).on("kt.menu.dropdown.show", function (item) {
                setTimeout(function () {
                    isActiveTskPopoverVisible = true;
                    document.querySelector("#Homepage_ActiveList .card-body").style.overflow = "hidden";
                }, 1);

                let itemData = item.dataset;
                _this.checkPopoverConfirmation(itemData.action, itemData.taskid, item, itemData.confirmmessage);
            });

            KTMenu.getInstance(item).on("kt.menu.dropdown.hide", function (item) {
                isActiveTskPopoverVisible = false;
                document.querySelector("#Homepage_ActiveList .card-body").style.overflow = "auto";
            });
        });
    ...
    getSendToUsers(taskId, taskType, keyValue, taskName, sendToDropDown) {
        let _this = this;
        ...
        this.callAPI(url, data, false, result => {
            if (result.success) {
                ...
                dataResult.forEach(function (item) {
                    ...
                    option.value = item.pusername;
                    option.text = item.pusername;
                    sendToDropDown.add(option);
                });
                sendToDropDown.classList.add("data-fetched");
            }
        });
    }
    ...
    checkComments(type, taskId, elem) {
        ...
        let taskProps = this.dataSources.TaskReasons[taskId];
    ...
    doSend(taskId, taskType, elem) {
        this.doTaskAction('SEND', taskId, taskType.toUpperCase(), elem);
    }
    ...
    doTaskAction(action, taskId, taskType, elem) {
        ...
        let toUser = "";
        if (action == "RETURN") {
            toUser = document.querySelector(`select[data-taskid="returnto_${taskId}"]${isMobile}`)?.value || '';
        }
        else if (action == "SEND") {
            toUser = document.querySelector(`select[data-taskid="sendto_${taskId}"]${isMobile}`)?.value || '';
        }

        if ((action == "RETURN" || action == "SEND") && (this.isUndefined(toUser) || this.isNullOrEmpty(toUser))) {
            showAlertDialog("Error", "Send To user is mandatory.");
            return;
        }
        ...
        try {
            doTask = AxBeforeTaskAction(this, action, taskId, taskType, elem);
        }
        catch (ex) {
            doTask = true;
        }
    ...
    showTasks() {
        ...
        document.querySelector(`#ActiveList_Container`).innerHTML = "";

        if (this.dataSources["Tasks"]?.length == undefined || this.dataSources["Tasks"].length == 0) {
            if (this.isUndefined(parent.axProcessObj)) {
                document.querySelector(`#ActiveList_Container`).insertAdjacentHTML("beforeend", ` ${this.noRecordsRowHtml} `);
                document.querySelector(".activelist-tabs").innerHTML = "";
            }
            else {
                document.querySelector(`#ActiveList_Container`).insertAdjacentHTML("beforeend", ` ${this.processedRowHtml} `);
                document.querySelector(".activelist-tabs").innerHTML = "";
            }
        }
        else {
            ...
            this.dataSources["Tasks"].forEach((rowData, idx) => {
                if (!this.isNullOrEmpty(rowData.taskstatus)) {
                    ...
                    document.querySelector(`#ActiveList_Container`).insertAdjacentHTML("beforeend", ` ${htmlText} `);
                    return;
                }
                ...
                switch (rowData.tasktype.toUpperCase()) {
                    ...
                    case "CHECK":
                        if ((rowData?.initiator_approval == "F" && rowData.touser == rowData.initiator) || rowData.showbuttons != "T") {
                            ...
                            rowData.task_buttons_html = this.taskBtnsHtml.check + this.taskBtnsHtml.return + (validSendFlgs.indexOf(rowData?.allowsendflg?.toString()) > -1 ? this.taskBtnsHtml.send : "") + this.taskBtnsHtml.view + this.taskBtnsHtml.history;
                    ...
                    case "APPROVE":
                        if ((rowData?.initiator_approval == "F" && rowData.touser == rowData.initiator) || rowData.showbuttons != "T") {
                            ...
                            rowData.task_buttons_html = this.taskBtnsHtml.approve + this.taskBtnsHtml.reject + (rowData?.returnable == 'T' ? this.taskBtnsHtml.return : "") + (validSendFlgs.indexOf(rowData?.allowsendflg?.toString()) > -1 ? this.taskBtnsHtml.send : "") + this.taskBtnsHtml.view + this.taskBtnsHtml.history;
                ...
                if (rowData?.rectype == "PEG" || this.isUndefined(rowData?.rectype) || this.isNullOrEmpty(rowData?.rectype)) {
                    rowData.displaymcontent = encodeURI(rowData.displaymcontent);

                    htmlText = Handlebars.compile(this.processRowHtml.replace("{{task_buttons_html}}", rowData.task_buttons_html).replace("{{mobile_task_buttons_html}}", rowData.mobile_task_buttons_html))(rowData);
                }
                else if (rowData?.rectype == "MSG" && rowData?.msgtype?.toUpperCase() == "REMINDER") {
                    htmlText = Handlebars.compile(this.reminderRowHtml)(rowData);
                }

                if (this.isAxpertFlutter) {
                    htmlText = htmlText.replaceAll('../custompages/icons', '../../icons');
                }

                htmlText = this.decodeHTMLEntities(htmlText);
            ...
            document.querySelectorAll('.HP-ActiveList-Items').forEach(div => {
                div.addEventListener('click', function () {
                    document.querySelectorAll('.HP-ActiveList-Items.active').forEach(div => {
                        div.classList.remove('active');
                    });

                    this.classList.add('active');
                    axTasksObj.toggleTask(this.dataset.taskid);
                });

                div.addEventListener('mouseover', function () {
                    if (isActiveTskPopoverVisible)
                        return;

                    document.querySelectorAll('.HP-ActiveList-Items.focused').forEach(focuseddiv => {
                        focuseddiv.classList.remove('focused');
                    });
                    div.classList.add('focused');

                    document.querySelectorAll('.HP-ActiveList-Items:not(.focused)').forEach(focuseddiv => {
                        focuseddiv.querySelectorAll('.show').forEach(popovers => {
                            popovers.classList.remove('show');
                        });
    ...
    constructFilters() {
        ...
        if (this.dataSources["Tasks"].length > 0) {
            var processWiseCounts = this.dataSources["Tasks"].reduce((p, c) => {
            ...
            for (const [key, value] of Object.entries(processWiseCounts)) {
                ...
                if (this.isAxpertFlutter) {
                    filterHtml = filterHtml.replaceAll('../custompages/icons', '../../icons');
                }
                document.querySelector(".activelist-tabs").insertAdjacentHTML("beforeend", ` ${filterHtml} `);
                if (tempSno == 6)
                    tempSno = 1;
                else
                    tempSno++;
            ...
            //this.dataSources["Tasks"].forEach((row) => {
        ...
        document.querySelectorAll(".nav-link").forEach((link) => {
            link.addEventListener("click", function () {
                document.querySelectorAll(".nav-link.active").forEach((activeItem) => {
    ...
    openTstruct(transId, recordId) {
        ...
        if (this.isAxpertFlutter) {
            ...
            url = `../aspx/tstruct.aspx?transid=${transId}`;
            if (!this.isNullOrEmpty(recordId) && recordId != "0")
    ...
    openHistory(processName, keyField, keyValue) {
        ...
        if (this.isAxpertFlutter) {
            this.checkAxpertConnection();
            url = `${axpertUrl}/aspx/AxMain.aspx?authKey=AXPERT-${armSessionId}&pname=iad___pth&params=~pkeyvalue=${keyValue}~pprocess=${processName}`;
        }
        else {
            url = `../aspx/ivtoivload.aspx?ivname=ad___pth&pkeyvalue=${keyValue}&pprocess=${processName}`;
        }
        try {
            let modalObj = {
                "id": `ldb${keyValue}`,
    ...
    openModal(modalObj) {
        try {
            let myModal = new BSModal(`modal_${modalObj.id}`, "", modalObj.iFrameModalBody,
                (opening) => { modalObj.opening() || ""; ShowDimmer(false); }, (closing) => { }
            );
    ...
    openProcess(processName, keyField, keyValue, taskId) {
        ...
        if (this.isAxpertFlutter) {
            this.checkAxpertConnection();
            window.location.href = `${axpertUrl}/aspx/AxMain.aspx?authKey=AXPERT-${armSessionId}&pname=hAxProcessFlow&params=~processname=${processName}~keyfield=${keyField}~keyvalue=${keyValue}&taskid=${taskId}`;
        }
        else {
    ...
    filterTasks(type, value, elem) {
        if (type == "process") {
            document.querySelectorAll(".HP-ActiveList-Items").forEach((task) => {
    ...
    toggleTask(taskId) {
        ...
        document.querySelectorAll(`.taskSubrow${taskId}`).forEach((view) => {
            ...
            if (!view.classList.contains("data-fetched")) {
                let contentHtml = view.dataset["displaymcontent"];
                if (contentHtml == "" || contentHtml == "null") {
                    ...
                    if (contentHtml.indexOf('{') > -1 || contentHtml.indexOf('[') > -1) {
                        let taskContainer = view.closest(".HP-ActiveList-Items");
                        let axHtmlObj = new AxHTML();
                        axHtmlObj.parse({ name: taskId, input: contentHtml, data: {}, pageVariables: { "taskid": taskId, "tasktype": taskContainer.getAttribute("data-type"), "transid": taskContainer.getAttribute("data-transid"), "keyvalue": taskContainer.getAttribute("data-keyvalue") }, container: `.taskSubrow${taskId} span` });
                    }
                    else {

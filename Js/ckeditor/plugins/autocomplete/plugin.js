	...
	Model.prototype = {
		...
		getIndexById: function( itemId ) {
			...
			for (var data = this.data, i = 0, l = data.length; i < l; i++) {
				let _ID = typeof data[i].id != "undefined" ? data[i].id : data[i].ID;
				if (_ID == itemId) {
					return i;
				}
			}
		...
		selectFirst: function() {
			if ( this.hasData() ) {
				this.select(typeof this.data[0].id != "undefined" ? this.data[0].id : this.data[0].ID);
			}
		},
		...
		selectLast: function() {
			if ( this.hasData() ) {
				this.select(typeof this.data[this.data.length - 1].id != "undefined" ? this.data[this.data.length - 1].id : this.data[this.data.length - 1].ID);
			}
		},
		...
		selectNext: function() {
			...
			if ( index < 0 || index + 1 == this.data.length ) {
				...
				this.select(typeof this.data[index + 1].id != "undefined" ? this.data[index + 1].id : this.data[index + 1].ID);
			}
		},

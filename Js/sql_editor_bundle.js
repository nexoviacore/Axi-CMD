//////////////////////////////////////////////////
// MANIKANTA                                    //
// It's not what you think that holds you back, //
// It's what you think you are not              //
//////////////////////////////////////////////////


/**
 * For SQL Editor following files are required in order to load dynamically
 * 1 -> codemirror/codemirror.js
 * 2 -> codemirror/addon/hint/show-hint.js
 * 3 -> codemirror/mode/sql.js
 * 4 -> codemirror/hint/sql_hint.js
 * 5 -> SQL_editor.js
 */
// ~~~~~~~codemirror/codemirror.js~~~~~~~~~~~~~START~~~~~~~~~~
...
if (typeof CodeMirror === "undefined") {
    ...
    (function(global, factory) {
        ...
        function coordsCharInner(cm, lineObj, lineNo, x, y) {
            ...
            for (;;) {
                ...
                if (middleX > x) {
                    to = middle;
                    toX = middleX;
                    if (toOutside = wrongLine) { toX += 1000; } dist = step
                } else {
                    from = middle;
                    fromX = middleX;
                    fromOutside = wrongLine;
        ...
        function onBlur(cm, e) {
            //customization
            if (cm.state.mousedownOnHint) {
                cm.state.mousedownOnHint = false;
                return;
            }
                    ...
                    var i = 0,
                        poll = function() {
                            if (display.selForContextMenu == cm.doc.sel && te.selectionStart == 0 &&
                                te.selectionEnd > 0 && input.prevInput == "\u200b") { operation(cm, selectAll)(cm) } else if (i++ < 10) { display.detectingSelectAll = setTimeout(poll, 500) } else { display.input.reset() }
                        }
...
// ~~~~~~~codemirror/codemirror.js~~~~~~~~~~~~~END~~~~~~~~~~~


//~~~~~~~~codemirror/addon/hint/show-hint.js~~~~~~~START~~~~~~~~~~
    ...
    (function(mod) {
        ...
        Completion.prototype = {
            ...
            pick: function(data, i) {
                ...
                CodeMirror.signal(this.cm, "pick", this.cm); //customization
                this.close();
            },
        ...
        function Widget(completion, data) {
            ...
            if(widget.data.list[0].text)                
                curCMEditorHints.html(expression_config.function[widget.data.list[0].text].selectText +"<br>"+ expression_config.function[widget.data.list[0].text].example);
            cm.addKeyMap(this.keyMap = buildKeyMap(completion, {
                moveFocus: function(n, avoidWrap) { 
                    widget.changeActive(widget.selectedHint + n, avoidWrap); 
                    if(widget.data.list[0].text)
                        curCMEditorHints.html(expression_config.function[widget.data.list[widget.selectedHint].text].selectText +"<br>"+ expression_config.function[widget.data.list[widget.selectedHint].text].example);
                },
                setFocus: function(n) { widget.changeActive(n); curCMEditorHints.html('');},
                menuSize: function() { return widget.screenAmount(); },
                length: completions.length,
                close: function() { completion.close(); curCMEditorHints.html('');},
                pick: function() { widget.pick(); curCMEditorHints.html('');},
                data: data
            }));
            ...
            CodeMirror.on(hints, "mousedown", function() {
                CodeMirror.signal(cm, "hint-mousedown", cm); //customization
...
//~~~~~~~~codemirror/addon/hint/show-hint.js~~~~~~~END~~~~~~~~~~~~


//~~~~~~~~~~codemirror/mode/sql.js~~~~~~~~~~~~~~START~~~~~~~~~~~~~~~

if (typeof CodeMirror !== "undefined" && (!CodeMirror.modes || !CodeMirror.modes.sql)) {
    // CodeMirror, copyright (c) by Marijn Haverbeke and others
    // Distributed under an MIT license: http://codemirror.net/LICENSE
...
//~~~~~~~~~~codemirror/mode/sql.js~~~~~~~~~~~~~~END~~~~~~~~~~~~~~~


//~~~~~~~~~~~~~codemirror/hint/sql_hint.js~~~~~~~~~~~~~START~~~~~~~~~~~~~~~~~
if (typeof CodeMirror !== "undefined" && (!CodeMirror.hint || !CodeMirror.hint.sql)) {
    // CodeMirror, copyright (c) by Marijn Haverbeke and others
    // Distributed under an MIT license: http://codemirror.net/LICENSE
    (function(mod) {
        if (typeof exports == "object" && typeof module == "object") // CommonJS
...
//~~~~~~~~~~~~~codemirror/hint/sql_hint.js~~~~~~~~~~~~~END~~~~~~~~~~~~~~~~~


//~~~~~~~~~~~~~~~~SQL_editor.js~~~~~~~~~~~~~~~~~~~~START~~~~~~~~~~~~~~~~~~~~~~

if (typeof createSQLeditor === "undefined") {

    (function() {
        createSQLeditor = function({ textarea, SQLhintObj, validateOnBlur, customValidationFn, nodeConfig }) {

            if (typeof textarea === "string") {
                textarea = textarea.trim();
                if (textarea.substr(0, 1) === "#") textarea = textarea.substr(1);
                textarea = document.getElementById(textarea);
            }

            const myEditor = CodeMirror.fromTextArea(textarea, {
                mode: 'text/x-sql',
                smartIndent: true,
                lineNumbers: true,
                matchBrackets: true,
            ...
            myEditor.on("blur", function(doc, changeObj) {
                let textArea = $(doc.getTextArea());
                textArea.val(doc.getValue()).blur();
                if (validateOnBlur) {
                    let textareaVal = doc.getValue();

                    if (typeof customValidationFn === "function") {
                        customValidationFn(textarea)
                    } 
                    // else if ($.trim(textareaVal) !== "") {
                    //     var settings = {
                    //         "async": true,
                    //         "crossDomain": true,
                    //         "url": nodeConfig.apiBase + "getQuery",
                    //         "method": "POST",
                    //         "headers": {
                    ...
                    //             "authorization": nodeConfig.authorization,
                    //             "appSKey": nodeConfig.appSKey,
                    //             "q": textareaVal,
                    //             "calledFrom": "validate",
                    //             "isMetadata": "true"
                    //         }
            ...
            myEditor.on("focus", function(doc, changeObj) {
                let textArea = $(doc.getTextArea());
                textArea.val(doc.getValue());
                try {
                    MainFocus(textArea);
                } catch (ex) {}
                curCMEditorHints = textArea.parent().find('.expHints');
            });

            
            myEditor.on('keyup', function(editor, event){
              if (
                !(event.ctrlKey) &&
            ...
            //myEditor.on("mousedown", function (doc, event) {
            //    myEditor.focus();
            //});

            //myEditor.on('hint-mousedown', function(editor) {
            //    editor.state.mousedownOnHint = true;
            //});
            ...
            $(textarea).data('myeditor', myEditor);
        }
